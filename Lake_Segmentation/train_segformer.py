# Train SegFormer model as two-class segmentation model
# This section implements a SegFormer model with metrics for evaluation and training callbacks
import os, random
import cv2, glob
import numpy as np
...
# Instantiate and compile SegFormer model
# Load the pre-trained MixTransformer (MiT) backbone preset
backbone = keras_cv.models.MiTBackbone.from_preset("mit_b0_imagenet")
backbone.trainable = True

base_model = keras_cv.models.segmentation.SegFormer(
    backbone=backbone,
    num_classes=2
)

model = tf.keras.Sequential([
    # Resize 256x256 inputs to 224x224 for SegFormer
    tf.keras.layers.Resizing(224, 224),
    base_model,
    # Resize the output predictions back to 256x256 to match reference masks
    tf.keras.layers.Resizing(256, 256, interpolation="bilinear")
])
...
ckpt_path = f'{base_dir}/model/segformer_model_best.keras'
callbacks = [
...
final_path = f'{base_dir}/model/segformer_model_final.keras'
